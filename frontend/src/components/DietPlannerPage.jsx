...
function MacroBar({ label, value, goal, unit = "g", color = "from-brand-blue to-emerald-400" }) {
  ...
  return (
    <div className="p-4 rounded-2xl bg-white/70 backdrop-blur-md border border-slate-200/80 shadow-sm">
      ...
      <div className="flex items-baseline gap-1.5 mb-2">
        <span className="font-display text-2xl font-bold text-brand-ink">{numVal}</span>
...
export default function DietPlannerPage({ onBack }) {
                ...
                {generatedPlan.meals.map((meal) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Add this single meal */}
                      <button
                        type="button"
                        onClick={() => handleApplySingleMeal(meal.category, meal)}
                        className="w-full py-2 rounded-xl bg-slate-100 hover:bg-brand-blue hover:text-white text-slate-700 text-xs font-bold transition-all border border-slate-200/80 cursor-pointer flex items-center justify-center gap-1.5"
                      >
                        <Plus className="h-3.5 w-3.5" />
                        Add {meal.category} to Today
                      </button>
                    </div>
                  );
                })}
